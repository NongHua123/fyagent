...
export function CodexFormFields({
  ...
  const isOfficialProvider =
    category === "official" || providerId === CODEX_OFFICIAL_PROVIDER_ID;
  // Native capabilities stay inside the existing advanced region. Opening a
  // provider never expands it automatically, even when a capability is on or
  // has a diagnostic; the collapsed hint remains the stable discovery point.
    ...
    <>
      ...
      {!isXaiOauthPreset && (
        <ApiKeySection
          ...
          category={isOfficialProvider ? "official" : category}
      ...
      {!isOfficialProvider && onModelChange && (
      ...
      {/* 高级选项始终包含 Codex 原生能力；其他设置仍仅面向非官方供应商。 */}
      <Collapsible
      ...
      >
        ...
        {!advancedExpanded && (
          <p className="mt-1 ml-1 text-xs text-muted-foreground">
            {isOfficialProvider
              ? t("codexConfig.officialAdvancedSectionHint", {
                  defaultValue: "包含内置生图扩展与 WebSocket 传输开关。",
                })
              : t("codexConfig.advancedSectionHint", {
                  defaultValue:
                    "包含 Codex 原生能力、上游格式、模型映射、思考能力与自定义 User-Agent。使用 Chat Completions 协议的供应商需开启路由接管才能使用。",
                })}
          </p>
        )}
        <CollapsibleContent className="space-y-3 pt-3">
          ...
          {!isOfficialProvider && shouldShowSpeedTest && !isXaiOauthPreset && (
          ...
          {!isOfficialProvider && isChatFormat && canEditReasoning && (
            <div
              className={cn(
                "space-y-3",
          ...
          {!isOfficialProvider && canEditCatalog && (
            <div
              className={cn(
                "space-y-4",
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  {t("codexConfig.modelMappingHint", {
                    defaultValue:
                      "选择模型角色后，FyAgent 会自动生成 Codex 兼容路由；菜单显示名可以填 DeepSeek、Kimi 等品牌模型，实际请求模型按右侧填写内容发送。",
                  })}
                </p>
              </div>
