...
requires_openai_auth = true
"#;
    fyagent_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(fyagent_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = fyagent_lib::get_codex_config_path();
    ...
    fyagent_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = fyagent_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        fyagent_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        fyagent_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        fyagent_lib::AppError::Config(msg) => {
    ...
    assert!(
        !fyagent_lib::get_codex_config_path().exists(),
...
"#;
    ...
    fyagent_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        fyagent_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    match err {
        ...
        fyagent_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = fyagent_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    fyagent_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = fyagent_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = fyagent_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".fyagent").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".fyagent");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.fyagent/settings.json
...
#[test]
fn import_sql_rejects_non_fyagent_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
...
#[test]
fn import_sql_accepts_fyagent_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("fyagent-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
