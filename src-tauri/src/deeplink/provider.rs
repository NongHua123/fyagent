...
//!
//! Handles importing provider configurations via fyagent:// URLs.
...
mod tests {
    ...
    fn grokbuild_deeplink_never_carries_env_key_or_resolved_secret() {
        let original = std::env::var_os("FYAGENT_DEEPLINK_ENV_PROBE");
        std::env::set_var("FYAGENT_DEEPLINK_ENV_PROBE", "secret-must-not-leak");
        ...
        let config = serde_json::json!({
            "config": concat!(
                ...
                "env_key = \"FYAGENT_DEEPLINK_ENV_PROBE\"\n",
        ...
        match original {
            Some(value) => std::env::set_var("FYAGENT_DEEPLINK_ENV_PROBE", value),
            None => std::env::remove_var("FYAGENT_DEEPLINK_ENV_PROBE"),
        }
    }
