...
7. [in progress] Open the closeout PR with real
   run/Release/digest/attestation evidence and the final native Windows
   x64/ARM64 locked uv/Python/Trellis Required smoke. After that PR's complete
   CI run succeeds, write the exact native evidence back, refresh the final
   design-package manifest, and archive the remaining children then parent in
   the same PR.
...
<https://github.com/NongHua123/fyagent/releases/tag/v0.3.0>. Steps 7–8 are
closeout operations and are not claimed complete by this active task record.
