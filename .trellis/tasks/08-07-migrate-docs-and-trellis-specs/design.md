...
The local commit series records implemented behavior first, then archives the
five old tasks as `superseded` in a separate history-preserving commit.
Release-dependent validation remains pending. After the public Release, a
closeout PR records real run URLs, asset summaries, digests, attestations, and
unsigned checks, refreshes `MANIFEST.sha256`, and enables the final child/parent
archive sequence.
