...
A6API (an aggregation platform that automatically picks the best of several upstreams for the same model) joins the preset catalog across eight apps; the PackyCode preset gains three backup addresses on the five preset types that support backup endpoints (Claude Code / Claude Desktop / Codex / Gemini CLI / Grok Build), selectable in the endpoint manager and the speed test; and the AICoding preset returns across seven apps.
...
### Provider Domains Refreshed

Several providers migrated domains, and their preset addresses and backup endpoints have been updated to match (PackyCode → `www.packyapi.ai`, RightCode → `www.rightapi.ai`, ClaudeAPI → `www.apito.ai`, APINebula → `apinebula.ai`, AICodeMirror → `.ai`, AICoding → `.inc`, AIGoCode → `.app`), along with the removal of two backup endpoints that had gone dead. **Already-created providers keep the old address stored in the database** — to move to the new domain, edit the address by hand or recreate the provider from the refreshed preset.
...
### Provider Domain Migrations Do Not Change Existing Providers
