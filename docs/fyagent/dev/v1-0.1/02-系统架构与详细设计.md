...
| `src/types.ts` | 前端 `ProviderMeta` | 增加 `imageExtensionConfigured?: boolean` |
| `src-tauri/src/provider.rs` | 后端 `ProviderMeta` | 增加同名 camelCase 序列化字段 |
...
```text
src-tauri/src/services/codex_provider_features.rs
```

对前端暴露两个不落盘的草稿命令：

```rust
analyze_codex_provider_features(toml_text, provider_id, api_format, meta)
patch_codex_provider_features(toml_text, provider_id, intent, api_format, meta)
```
...
interface CodexProviderFeatureState {
  ...
  imageExtension:
    ...
    | { kind: "conflict"; key: string; value: string }
...
2. 无同名键：
   - `imageExtensionConfigured == true` → `off`；
   - 元数据缺失且为历史适用供应商 → `legacyPendingOn`；
   - 新建草稿 → 默认注入后为 `on`。
3. 恰好一个同名键且值为 `local-image-extension` → `on`。
4. 同名键值不同，或存在多个同名语义键且值不一致 → `conflict`。
5. TOML 结构不是 map/string → `invalid`。

冲突时不得自动写入或删除。保存其他字段时，若用户未解决冲突：
...
- 有其他头时只插入目标键；
- 已有目标值时幂等；
- 冲突时返回结构化冲突，不返回被修改 TOML。

**关闭生图：**

- 只删除值匹配的同名键；
- 空 `http_headers` 是否删除整个字段由原有样式决定：若是本功能创建且已空，可删除；若存在装饰/注释，保留空表以免丢注释；
- 冲突值不删除。

**WebSocket：**

- Responses + 开启：设置布尔 `true`；
- 关闭：删除字段；
- 非 Responses + 开启：返回不兼容错误；
- 切换 API 格式离开 Responses：在本地草稿中移除字段，并展示一次说明。
...
只有用户成功保存该供应商且不存在未解决冲突时，才写 `true`。实际开关状态仍以 TOML 为准；元数据仅解决“历史未迁移”和“明确关闭”都表现为缺少请求头的歧义。
...
- Codex 官方将 `http_headers` 和 `supports_websockets` 定义在 `model_providers.<id>`，WebSocket 字段对应 Responses transport。
