...
三方比较时只能把单条仍有效意图作为审计输入，并以当前文件、测试、覆盖决策和 child evidence 决定是否移植。本轮 overlay diff 只包含上述 3 个中央冻结声明；108 个 payload 字节未修改。原始 `MANIFEST.sha256` 因当前设计包修订而保持 stale，直到正式 Release closeout 后统一重建。
