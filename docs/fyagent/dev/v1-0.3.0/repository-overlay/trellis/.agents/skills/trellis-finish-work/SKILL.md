...
Filter out paths under `.trellis/workspace/` and `.trellis/tasks/` — those are managed by `mise run trellis:session:add` and `mise run trellis:task -- archive` auto-commits and will appear dirty as part of this skill's own work.
