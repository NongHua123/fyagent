...
`package.json` remains the source of runnable frontend scripts and its `packageManager` field owns the pnpm version. `.node-version` owns the Node.js version. mise consumes those standard declarations and exposes the canonical repository tasks without duplicating their version values.
...
All current project checks and tests use canonical `mise run` tasks. The [Quality Guidelines](./quality-guidelines.md) own the native Fetch/DEP0040 regression contract and the distinction between mock-only desktop evidence and real platform evidence.
