...
> **FyAgent project command boundary**  
> Repository Trellis operations use `mise run trellis:*`. mise selects uv; uv selects the project-managed Python and `.venv`. Do not substitute system `python3`, `python`, or `py`. `mise trust` remains an explicit user action. Generic Trellis architecture references may show native Python examples, but they are not the FyAgent operational entry point.
...
```bash
mise run trellis:init-developer <your-name>
```
...
```bash
mise run trellis:context --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
mise run trellis:task -- create "<title>" [--slug <name>] [--parent <dir>]
mise run trellis:task -- start <name>          # set active task (session-scoped when available)
mise run trellis:task -- current --source      # show active task and source
mise run trellis:task -- finish                # clear active task (triggers after_finish hooks)
mise run trellis:task -- archive <name>        # move to archive/{year-month}/
mise run trellis:task -- list [--mine] [--status <s>]
mise run trellis:task -- list-archive

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
mise run trellis:task -- add-context <name> <action> <file> <reason>
mise run trellis:task -- list-context <name> [action]
mise run trellis:task -- validate <name>

# Task metadata
mise run trellis:task -- set-branch <name> <branch>
mise run trellis:task -- set-base-branch <name> <branch>    # PR target
mise run trellis:task -- set-scope <name> <scope>

# Hierarchy (parent/child)
mise run trellis:task -- add-subtask <parent> <child>
mise run trellis:task -- remove-subtask <parent> <child>

# PR creation
mise run trellis:task -- create-pr [name] [--dry-run]
```

> Run `mise run trellis:task -- --help` to see the authoritative, up-to-date list.

**Current-task mechanism**: `task.py create` creates the task directory and (when session identity is available) auto-sets the per-session active-task pointer so the planning breadcrumb fires immediately. `mise run trellis:task -- start` writes the same pointer (idempotent if already set) and flips `task.json.status` from `planning` to `in_progress`. State is stored under `.trellis/.runtime/sessions/`. If no context key is available from hook input, `TRELLIS_CONTEXT_ID`, or a platform-native session environment variable, there is no active task and `mise run trellis:task -- start` fails with a session identity hint. `task.py finish` deletes the current session file (status unchanged). `task.py archive <task>` writes `status=completed`, moves the directory to `archive/`, and deletes any runtime session files that still point at the archived task.
...
```bash
mise run trellis:session:add -- --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
mise run trellis:context                            # full session runtime
mise run trellis:context --mode packages            # available packages + spec layers
mise run trellis:context --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
- Lightweight tasks may be PRD-only. Complex tasks must have `prd.md`, `design.md`, and `implement.md` before `mise run trellis:task -- start`.
...
- 1.4 Activate task `[required · once]` (review gate, then `mise run trellis:task -- start`; status → in_progress)
...
Lightweight: `prd.md` can be enough. Complex: finish `prd.md`, `design.md`, and `implement.md`; ask for review before `mise run trellis:task -- start`.
...
- Task creation approval is not implementation approval; implementation waits for `mise run trellis:task -- start` after artifact review.
...
```bash
mise run trellis:context --mode phase --step <step>
# e.g. mise run trellis:context --mode phase --step 1.1
```
...
```bash
mise run trellis:task -- create "<task title>" --slug <name>
```
...
Skip when `mise run trellis:task -- current --source` already points to a task.
...
```bash
mise run trellis:task -- add-context "$TASK_DIR" implement "<path>" "<reason>"
mise run trellis:task -- add-context "$TASK_DIR" check "<path>" "<reason>"
```

Delete the seed `_example` line once real entries exist (optional — it's skipped automatically by consumers).

Ready gate: both `implement.jsonl` and `check.jsonl` must contain at least one real `{"file": "...", "reason": "..."}` entry before `mise run trellis:task -- start`. The seed `_example` row alone is not ready.
...
```bash
mise run trellis:task -- start <task-dir>
```
...
If `mise run trellis:task -- start` errors with a session-identity message (no context key from hook input, `TRELLIS_CONTEXT_ID`, or platform-native session env), follow the hint in the error to set up session identity, then retry.
...
| `mise run trellis:task -- start` has been run (status = in_progress) | ✅ |
...
**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `mise run trellis:context --mode packages`, then load each package's spec index Quality Check section. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
