...
FyAgent のローカル開発では、ツールのバージョン管理とリポジトリのタスク実行に
グローバルな [mise](https://mise.jdx.dev/) を 1 つ使用します。設定内容を確認してから
明示的に信頼してください。
...
`bootstrap` は個人の mise ポリシーに従って Node/Rust/uv を準備し、固定された
pnpm 依存関係と uv の Python 環境を同期して、厳格な環境・タスク検査を行います。
自動インストールを個人設定で無効にしている場合は、先に `mise install` を実行します。
...
| uv | `latest`、承認済み実体は lock で固定 | `mise.toml` + `mise.lock` |
| Python | `3.14.7`、uv のみが管理 | `.python-version` |

Tauri のネイティブコンパイラ、WebView、ライブラリはホスト OS のパッケージ管理に
属します。`mise run system:check` は不足項目と公式手順を表示しますが、権限昇格して
インストールしません。
...
保守対象のローカル操作は `mise run <task>` を使用します。詳細は
[docs/fyagent/development/mise-tasks.md](docs/fyagent/development/mise-tasks.md) を参照してください。

### ネイティブビルドと正式リリース

ローカルビルドは現在のホスト OS とアーキテクチャのみを対象とします。従来の
Linux/WSL から Windows/macOS へのローカルクロスビルドは廃止されます。各 OS の
開発者は引き続きネイティブ開発・診断ビルドを行えます。

Windows x64/ARM64、Linux x64/ARM64、macOS Universal の正式成果物は GitHub
Actions のみがビルド、署名/公証、検証、公開します。ローカル bundle は正式な
FyAgent Release 資産ではありません。
...
mock/visual preflight はネイティブ installer の証拠ではありません。visual baseline
更新は明示的な変更タスクで、レビュー済み候補環境の証拠が必要です。
...
- フォーマットチェック: `mise run format:check`
- 単体テスト: `mise run test:unit`
