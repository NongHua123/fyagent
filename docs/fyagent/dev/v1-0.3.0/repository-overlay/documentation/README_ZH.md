...
FyAgent 在本地仅要求全局安装一份 [mise](https://mise.jdx.dev/)，用于统一工具版本和
仓库任务入口。请先审阅项目配置，再显式信任：
...
`bootstrap` 会根据个人 mise 策略安装或选择项目声明的 Node、Rust、uv，同步冻结的
pnpm 依赖和由 uv 锁定的 Python 环境，并执行严格环境/任务检查。若个人配置关闭了
mise 自动安装，请先运行 `mise install`。
...
| uv | `latest` 选择器，批准版本由锁文件固定 | `mise.toml` + `mise.lock` |
| Python | `3.14.7`，仅由 uv 管理 | `.python-version` |

Tauri 的系统编译器、WebView 和原生库仍由宿主包管理器负责。`mise run system:check`
只报告当前平台缺少的依赖和官方安装方式，不执行提权安装。

### 标准开发任务

```bash
mise run dev                         # Tauri 桌面开发（交互式）
mise run dev:renderer                # 仅启动 Vite
mise run build:renderer              # 仅构建前端
mise run build:binary                # 当前宿主 Tauri 二进制，不打包
mise run build                       # 当前宿主原生安装包/Bundle
mise run build:debug                 # 当前宿主 Debug 构建
...
当前维护的本地项目操作统一使用 `mise run <task>`。完整任务参考见
[docs/fyagent/development/mise-tasks.md](docs/fyagent/development/mise-tasks.md)。

### 原生构建与正式发布边界

本地构建只支持当前宿主系统和架构。原 Linux/WSL 到 Windows、macOS 的本地交叉
构建命令已经退役；Windows、macOS、Linux 开发者仍可在各自设备上进行原生开发和
诊断构建。

Windows x64/ARM64、Linux x64/ARM64 和 macOS Universal 的正式产物只允许由
GitHub Actions 构建、签名/公证、校验和发布。本地产物不是正式 FyAgent Release 资产。
...
桌面 mock 与视觉预检不能替代真实平台或安装包证据；视觉基线更新是显式修改型任务，
必须附带经评审的候选环境证据。
...
- 通过类型检查：`mise run typecheck`
- 通过格式检查：`mise run format:check`
- 通过单元测试：`mise run test:unit`
