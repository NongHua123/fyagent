...
FyAgent requires one globally installed [mise](https://mise.jdx.dev/) for local
version selection and the repository task API. Review the project configuration,
then explicitly trust and bootstrap it:
...
`bootstrap` uses the developer's mise auto-install policy to install or select
the declared Node.js, Rust, and uv tools, synchronizes frozen pnpm dependencies
and the uv-locked Python environment, then runs strict environment and task
checks. If personal mise configuration disables automatic installation, run
`mise install` first.

| Tool | Project baseline | Single version source |
| --- | --- | --- |
| Node.js | `24.19.0` | `.node-version` |
| pnpm | `10.12.3` | `package.json#packageManager` |
| Rust | `1.97.1` | `rust-toolchain.toml` |
| uv | `latest` selector; approved version locked | `mise.toml` + `mise.lock` |
| Python | `3.14.7`, managed only by uv | `.python-version` |

Tauri compilers, WebView components, and native libraries remain host-system
prerequisites. `mise run system:check` reports missing requirements and official
installation guidance; it does not perform privileged installation.

### Canonical Development Tasks

```bash
mise run dev                         # interactive Tauri desktop development
mise run dev:renderer                # Vite renderer only
mise run build:renderer              # renderer production build
mise run build:binary                # native Tauri binary, no bundle
mise run build                       # current-host native bundle/installer
mise run build:debug                 # current-host debug build
...
Maintained local project operations use `mise run <task>`. See the proposed
[task reference](docs/fyagent/development/mise-tasks.md) for the complete API.

### Native Build and Formal Release Boundary

Local builds support only the current host operating system and architecture.
The previous Linux/WSL-to-Windows and Linux/WSL-to-macOS cross-build commands are
retired. Windows, macOS, and Linux developers can continue native development
and diagnostic builds on their own devices.

Formal Windows x64/ARM64, Linux x64/ARM64, and macOS Universal artifacts are
built, signed/notarized, verified, and published only by GitHub Actions. Local
bundles are not formal FyAgent Release assets.

### Rust Backend and Tests
...
Desktop mock and visual-preflight checks do not replace real-platform or
installer evidence. Visual-baseline updates are explicit modifying operations
and require reviewed candidate-environment evidence.
...
- Pass type check: `mise run typecheck`
- Pass format check: `mise run format:check`
- Pass unit tests: `mise run test:unit`
