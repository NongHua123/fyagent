...
|   13 | PR #8：`623b6924` → `4645668d`                                                                     | 首败 `31264604075`；修复 run `31265504901` 全绿；manifest 已重建 → archive/journal。 | manifest 前不归档；final CI 前不合并。   |

实现 PR、Release、closeout native smoke 与最终 manifest 已有真实证据；closeout PR 应继续引用覆盖决策、Trellis child、风险项与已发生证据。仍未发生的 archive/journal、final PR CI/merge、exact-main CI 与分支清理不得使用占位值或预期成功表述。
