...
本表由原始目标矩阵迁移为当前实施 inventory。A–E 中已有实现 commit 的行表示已执行；PR/main/preflight/formal Release 已由真实运行和附件证据验证，closeout PR native smoke 也已由 run `31265504901` 远程验证，最终设计包 manifest 已重建并复验。archive/journal、final PR CI/merge、exact-main CI 和分支清理仍以 Pending 为准，不能从文件存在推断完成。
...
| 新 parent + 6 child tasks                                   | 已物化并按序执行                | Parent/Child 6 | Child1/2/5 已归档；native smoke 已通过，Child3/4/6 与 parent 待按序归档 |
