...
本表是后续真实仓库实施的文件所有权与边界清单。`Action` 表示目标动作，不表示本交付已执行该动作。最终 diff 必须在完整合并 `v3.19.2` 后重新生成并逐项对照。
...
| `scripts/tasks/*.mjs` | 新增 | Child 3/4/6 | 复杂跨平台逻辑；可单测；不得以 Bash 承担通用核心逻辑 |
| `package.json#scripts` | 保留包级实现并补必要脚本 | Child 3 | mise task 包装而非反向调用；可增加 `build:debug`/contract scripts |
| `.codex/hooks.json` | 后续真实实施修改 | Child 3/6 | 仅引用 `mise run --silent codex:hook:*`；schema 按实际 Codex 版本复核 |
...
| `.github/workflows/release.yml` | 重写/拆分 | Child 4 | unsigned/signed/formal 模式、5 target groups、10 assets、失败关闭 publish |
...
| Branch ruleset | 仓库管理员实施 | Child 4 | `main` 要求稳定 Required context；Merge Queue 合同 |
| Tag ruleset | 仓库管理员实施 | Child 4 | `v*` 禁止删除/覆盖 |
| Release environments/secrets | 仓库管理员实施 | Child 4 | signed/formal 审批、签名 secrets、最小可见范围 |
...
| `.trellis/spec/backend/development-environment.md` | 重写 | Child 6 | 标准版本文件、mise/uv/Python/locks/strict checks |
| `.trellis/spec/backend/task-runner-contract.md` | 新增 | Child 6 | task API、参数、副作用、弃用、文档生成 |
| `.trellis/spec/backend/upstream-sync.md` | 新增 | Child 6 | fork/upstream、merge、许可和可选 runtime mise |
| `.trellis/spec/backend/development-hooks.md` | 新增 | Child 6 | Trellis/Codex wrapper 与可见降级 |
| `.trellis/spec/backend/github-release-workflow.md` | 重写 | Child 6 | Required/Release 长期合同 |
| `.trellis/spec/backend/windows-release-boundary.md` | 重写 | Child 6 | native MSI、manifest 分层、签名/安装安全 |
| `.trellis/spec/frontend/quality-guidelines.md` | 重写/增补 | Child 6 | Native Fetch、DEP0040、测试证据边界 |
| backend/frontend `index.md` | 更新 | Child 6 | 保留所有仍有效规范并加入新规范；移除退役项 |
| 新 parent + 6 child tasks | 新增 planning artifacts | Child 6 | PRD/design/implement/JSONL 完整、关系一致 |
| 当前 5 个 task 目录 | 归档为 superseded | Child 6 | 原文保留、先 child 后 parent、`--no-commit`、不称已完成 |
...
- FyAgent 品牌、bundle/deep-link/data path、`FYAGENT_*`、独立 `0.2.x` 版本线和混合许可边界；
