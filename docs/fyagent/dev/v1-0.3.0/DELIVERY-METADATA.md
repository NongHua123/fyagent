...
| mise/uv/task/version      | `3d534710`                         | `8bd54f6b`; closeout smoke pending             | release runners verified; Windows locked uv/Trellis PR gate pending |
...
- 当前 `MANIFEST.sha256` 仍对应原设计包快照，不能证明正在修改的目录。closeout native evidence 写回且本设计包字节稳定后、live `.trellis/tasks` archive 之前才重新生成；任务归档目录不在该清单范围内，清单自身也不参与哈希以避免自引用。
