...
[Observed / 已核实] 当前上传基线同时存在：Linux/WSL 本地交叉构建、仅手动触发的 CI、Node/Rust 本地与 Actions 漂移、非规范的 mise 任务入口、系统 Python 直调、`cross-fetch` 引入的旧 `punycode` 依赖链，以及活动文档/Trellis 规范与目标架构冲突。
...
- **REL-001**：正式 tag 必须与产品版本一致并指向受保护 main 的已通过提交。
...
- **REL-005**：生成下载清单、SHA-256、构建元数据和可用时的 artifact attestation。
...
| uv | `latest` 选择器 / lock 精确版本 | `mise.toml` + `mise.lock` | Actions 不使用 uv，除非未来 CI 运行 Python/Trellis 检查时另行设计 |
| Python | 3.14.7 | `.python-version` | 本地由 uv 管理；是否在 Actions 运行 Python 合同由实际 CI 设计决定 |

## 6. 验收总则

- 所有 `D-001`–`D-104` 可追踪到设计、文件、子任务和证据；
- 合并、配置、依赖、CI 和 Release 结果必须在后续实施中产生真实证据；
- 不得用“默认无告警”“编译成功”或“某平台静态检查”替代明确的跨平台/签名/依赖图验收；
