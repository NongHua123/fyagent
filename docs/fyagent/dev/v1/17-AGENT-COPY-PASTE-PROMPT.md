...
status: final
version: v1
usage: copy selected prompt into Codex agent session
---
...
```text
你正在私有仓库中实施 FyAgent V1“一键安装 Codex”。先完整阅读 v1/00-AGENT-START-HERE.md 至 v1/18-REFERENCES.md，并把这些文档视为实现契约。
...
采用 worktree并行：Core先冻结领域/IPC契约；Windows、macOS、UI在该契约上并行；最终由你按 Core→Windows→macOS→UI顺序 cherry-pick，并独占 lib.rs、store.rs、commands/mod.rs、services/mod.rs、Cargo files、品牌/updater和旧 Codex CLI清理。
...
```text
你负责 FyAgent V1 Codex Desktop Installer 的 Core worktree。只修改 v1/03-AGENT-WORKTREE-EXECUTION.md 分配给 Core 的文件。先阅读 01、02、04、05、06、10、11、12、13。
...
```text
你负责 Windows platform worktree。基于已冻结Core契约，阅读 02、05、06、07、10、11、13、14。
...
```text
你负责 macOS platform worktree。基于Core契约，阅读 02、05、06、08、10、11、13、14。
...
```text
你负责 UI worktree。阅读 02、05、09、10、11、13。
