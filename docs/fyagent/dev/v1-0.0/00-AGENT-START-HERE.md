# FyAgent V1.0.0 — 初版设计归档

> 文档状态：历史初版设计记录，保留原始决策与当时的协作约束；不是当前实现的永久冻结基线。
> 当前契约：与本目录不兼容或被后续设计覆盖的行为，以 [`../v1-0.2/README.md`](../v1-0.2/README.md) 及其引用的 v1.0.2 文档为准。
...
本归档记录了当时建议的阅读顺序。新实现不得只据此直接编码，应先阅读 v1.0.2 当前文档，再按需回溯本目录理解历史背景：
