...
本文件记录 FyAgent V1.0.0 当时冻结的后端领域模型、安装任务状态机、前后端 IPC 形态和并发不变量。开发 Agent 可以根据实际 Rust/TypeScript 代码风格调整字段命名，但不得改变本文定义的语义。它只用于追溯初版设计；与 v1.0.2 当前契约冲突时，以 [`../v1-0.2/README.md`](../v1-0.2/README.md) 为准。
