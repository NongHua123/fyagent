...
Signing and notarization are release-specific. Download from the [Releases page](https://github.com/NongHua123/cc-switch/releases) and verify the evidence published for the exact FyAgent build before installing it.
...
2. Add FyAgent to your antivirus software's whitelist
...
chmod +x FyAgent-*.AppImage

# If it still fails, try
./FyAgent-*.AppImage --no-sandbox
```
...
**Solution**: Use the dedicated `FYAGENT_GDK_BACKEND` environment variable to switch back to native Wayland (it is read before GTK init, and the hook never overrides it):

```bash
FYAGENT_GDK_BACKEND=wayland ./FyAgent-*.AppImage
```

- When launching from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env FYAGENT_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment — otherwise an icon launch won't see the variable.
- The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, set `FYAGENT_GDK_BACKEND=x11` instead.
...
**Solutions**:
1. Check if the `~/.fyagent/` directory exists
2. Restore from backup: `~/.fyagent/backups/`
...
**Solutions**:
1. Confirm the file was exported by FyAgent
...
**Windows**:
- Check taskbar settings to ensure the FyAgent icon is not hidden
...
2. Restart the app
3. Delete `~/.fyagent/settings.json` to reset settings
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and FyAgent runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, FyAgent covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `fyagent://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/NongHua123/cc-switch/issues)
...
FyAgent stores logs under its application configuration directory. By default this is `.fyagent` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.fyagent/logs/fyagent.log` and its rotated files
- Application crashes: `~/.fyagent/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.fyagent\...`
