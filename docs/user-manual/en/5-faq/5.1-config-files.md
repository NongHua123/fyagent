...
## FyAgent Data Storage

### Storage Directory

Default location: `~/.fyagent/`
...
```
~/.fyagent/
├── fyagent.db      # SQLite database (SSOT)
...
`fyagent.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. FyAgent writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to FyAgent's "providers") |
...
FyAgent's priority when modifying configurations:

1. **FyAgent Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by FyAgent)
- FyAgent's `settings.json`

### Not Recommended to Edit Manually

- `fyagent.db` database file
- Backup files
...
FyAgent v3.7.0 migrated from JSON files to SQLite:
